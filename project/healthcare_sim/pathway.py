...
class Pathway:
    ...
    def next_action(self, patient, actions, q_table, epsilon, major_step, step, activity_log, system_state):
        ...
        if major_step == 0:
            ...
            # Use guided policy if available, else random
            if hasattr(self, 'policy') and self.policy is not None and q_state in self.policy:
                guided_action = self.policy[q_state]
                if guided_action in valid_actions:
                    next_a = guided_action
                else:
                    next_a = random.choice(valid_actions)
            else:
                next_a = random.choice(valid_actions)
